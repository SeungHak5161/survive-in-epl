...
const PredictSimple = ({
  ...
  const [predictList, setPredictList] = useState<IPredict[] | undefined>(undefined)
  useEffect(() => {
    if (isLoading || !predictData) return
    const newPredictList = predictData.slice(0, 5)
    ...
    <div className="rounded-xl bg-white p-3 pb-1">
      {isLoading || !predictData ? (
        ...
        <div className="flex flex-col gap-1">
          ...
          {predictList &&
            predictList.map((obj, idx) => {
              return (
                <div
                  key={idx}
                  className={`grid grid-cols-4 items-center py-2 ${idx === predictList.length - 1 || "border-b-[1px] border-b-slate-200"}`}
                >
